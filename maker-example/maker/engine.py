...
class Engine(BaseEngine):
    ...
    def eval(self) -> bool:
        ...
        for order in self.open_orders.values():
            ...
            edge = math.fabs(self.theo - price) if order.side == "buy" else math.fabs(price - self.theo)
            if edge < self.config.min_edge:
                logging.info("%s @ %.2f, edge=%.3f, cancelling...", order.side, price, edge)
                self.cancel_order(order)
            else:
                logging.info("%s @ %.2f, edge=%.3f", order.side, price, edge)

        # fill in missing levels
        price = float(self.open_buys[-1].price) - self.config.min_tick if len(self.open_buys) > 0 else self.theo - self.config.min_edge
        ...
        price = float(self.open_sells[-1].price) - self.config.min_tick if len(self.open_sells) > 0 else self.theo + self.config.min_edge
